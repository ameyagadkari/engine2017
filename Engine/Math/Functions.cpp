...
uint16_t eae6320::Math::ConvertFloatToHalf( const float i_value )
{
    ...
    const union
    {
        float valueAsFloat;
        uint32_t valueAsInt;
    } bitRepresentation = { i_value };
    // The first bit is the sign
    const auto sign_float = bitRepresentation.valueAsInt & 0x80000000;
    const auto sign_half = static_cast<uint16_t>( sign_float >> 16 );
    // The next 8 bits are the exponent
    // (a half has 5 bits)
    const auto exponent_float = bitRepresentation.valueAsInt & 0x7f800000;
    // The final 23 bits are the significand
    // (a half has 10 bits)
    const auto significand_float = bitRepresentation.valueAsInt & 0x7fffff;
    // Check for values that are out of range
    const auto absoluteValue_float = bitRepresentation.valueAsInt & 0x7fffffff;
    constexpr uint32_t maxHalfValue_float = 0x477FE000;    // Exponent == 15, Significand = 0b1111111111 shifted left (23 - 10)
...
{
    return 2.0f * atan( tan( i_horizontalFieldOfView_inRadians * 0.5f ) / i_aspectRatio );
}
