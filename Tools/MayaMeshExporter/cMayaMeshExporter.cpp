...
MStatus eae6320::cMayaMeshExporter::writer(const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode)
{
	...
	std::vector<sMaterialInfo> materialInfo;
	{
		...
		std::vector<MObject> shadingGroups;
		{
			...
			else if (i_mode == kExportActiveAccessMode)
			{
...
namespace
{
		...
		std::vector<sMaterialInfo>& o_materialInfo)
	{
		...
		std::map<std::string, size_t> vertexKeyToIndexMap;
		{
			...
			for (auto i = as_const(i_uniqueVertices).begin(); i != i_uniqueVertices.end(); ++i)
			{
				sortedVertices.insert(make_pair(i->second, i->first));
			}
			...
			for (auto i = sortedVertices.begin(); i != sortedVertices.end(); ++i, ++vertexIndex)
			{
				...
				vertexKeyToIndexMap.insert(make_pair(i->second, vertexIndex));
		...
		{
			// Sort the triangles by shading group
			sort(io_triangles.begin(), io_triangles.end(), sTriangle::CompareTriangles);
			...
			if (status)
			{
					...
					for (unsigned int i = 0; i < shadingGroups.length(); ++i)
					{
						size_t shadingGroupIndex;
						{
							...
							else
							{
								...
								io_map_shadingGroupNamesToIndices.insert(make_pair(shadingGroupName, newIndex));
				...
				// Convert each polygon shading group index
				{
					...
					if (polygonCount == mesh.numPolygons())
					{
						...
						{
							const auto localIndex = localIndices[i];
							if (localIndex >= 0)
							{
			...
			for (MItMeshPolygon i(mesh.object()); !i.isDone(); i.next(), ++polygonIndex)
			{
				if (i.hasValidTriangulation())
				{
					...
					std::map<int, const std::string> indexToKeyMap;
					{
						...
						if (status)
						{
							for (unsigned int j = 0; j < vertices.length(); ++j)
							{
								...
								indexToKeyMap.insert(make_pair(positionIndex, vertexKey));
								io_uniqueVertices.insert(make_pair(vertexKey,
					...
					// Store information for each individual triangle in the polygon
					{
						...
						for (auto j = 0; j < triangleCount; ++j)
						{
		...
		const std::vector<sMaterialInfo>& i_materialInfo)
	{
		...
		if (fout.is_open())
		{
			...
			// Making the file stream to output only fixed floating point numbers with six decimal places
			fout.precision(6);
			fout << std::fixed;

			const auto vertexCount = i_vertexArray.size();
			const auto indexCount = i_indexArray.size();
			...
			fout <<
				...
				"\t{\n"
				"\t\t-- Position table contains positions(x,y,z)\n"
				"\t\t-- Color table contains colors(r,g,b,a)(a has a default value of 1.0 if you don't specify)\n";

			for (size_t i = 0; i < vertexCount; ++i)
			{
				fout <<
					// Opening per-vertex data table				
					"\t\t{-- " << i << "\n"

					"\t\t\tposition = "
					// Opening position table
					"{" <<
					i_vertexArray[i].vertex.x << "," <<
					...
					// Opening color table
					"{" <<
					i_vertexArray[i].vertex.r << "," <<
					i_vertexArray[i].vertex.g << "," <<
					i_vertexArray[i].vertex.b << "," <<
					i_vertexArray[i].vertex.a <<
					// closing color table
					"},\n"
					...
					// Opening uv table
					"{" <<
					i_vertexArray[i].vertex.u << "," <<
					i_vertexArray[i].vertex.v <<
					// closing uv table
					"},\n"

					"\t\t\tnormal = "
					// Opening normal table
					"{" <<
					i_vertexArray[i].vertex.nx << "," <<
					i_vertexArray[i].vertex.ny << "," <<
					i_vertexArray[i].vertex.nz <<
					// closing normal table
					"},\n"

					"\t\t\ttangent = "
					// Opening tangent table
					"{" <<
					i_vertexArray[i].vertex.tx << "," <<
					i_vertexArray[i].vertex.ty << "," <<
					i_vertexArray[i].vertex.tz <<
					// closing tangent table
					"},\n"

					"\t\t\tbitangent = "
					// Opening bitangent table
					"{" <<
					i_vertexArray[i].vertex.btx << "," <<
					i_vertexArray[i].vertex.bty << "," <<
					i_vertexArray[i].vertex.btz <<
					// closing bitangent table
					"},\n"

					// closing per-vertex data table				
					"\t\t},\n";
			}
			...
			fout <<
				...
				"\t-- Total number of indices: " << indexCount << "\n"
				"\t-- Total number of triangles: " << indexCount / 3 << "\n"
				"\tindices =\n"
