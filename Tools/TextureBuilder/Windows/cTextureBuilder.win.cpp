...
namespace
{
		...
		DirectX::ScratchImage &o_texture)
	{
		...
		if (DirectX::IsCompressed(io_sourceImageThatMayNotBeValidAfterThisCall.GetMetadata().format))
		{
			...
			const auto result = Decompress(io_sourceImageThatMayNotBeValidAfterThisCall.GetImages(), io_sourceImageThatMayNotBeValidAfterThisCall.GetImageCount(),
		...
		DirectX::ScratchImage resizedImage;
		{
			...
			auto targetHeight = flippedMetadata.height;
			{
				...
				// ("BC" stands for "block compression", and each block is 4x4)
				{
					...
					targetWidth = eae6320::Math::RoundUpToMultiplePowerOf2(targetWidth, blockSize);
					targetHeight = eae6320::Math::RoundUpToMultiplePowerOf2(targetHeight, blockSize);
				}
			...
			if ((targetWidth != flippedMetadata.width) || (targetHeight != flippedMetadata.height))
			{
				...
				const auto result = Resize(flippedImage.GetImages(), flippedImage.GetImageCount(),
		...
		DirectX::ScratchImage imageWithMipMaps;
		{
			...
			if (!resizedImage.GetMetadata().IsVolumemap())
			{
				result = GenerateMipMaps(resizedImage.GetImages(), resizedImage.GetImageCount(),
					resizedImage.GetMetadata(), useDefaultFiltering, generateAllPossibleLevels, imageWithMipMaps);
			}
			else
			{
				result = GenerateMipMaps3D(resizedImage.GetImages(), resizedImage.GetImageCount(),
					resizedImage.GetMetadata(), useDefaultFiltering, generateAllPossibleLevels, imageWithMipMaps);
			}
			if (FAILED(result))
			{
		...
		// Compress the texture
		{
			...
			const auto useDefaultThreshold = DirectX::TEX_THRESHOLD_DEFAULT;
			const auto result = Compress(imageWithMipMaps.GetImages(), imageWithMipMaps.GetImageCount(),
	...
	eae6320::cResult LoadSourceImage(const char *const i_path, DirectX::ScratchImage &o_image)
	{
		...
		{
			const auto extension = path.substr(path.find_last_of(L'.') + 1);
	...
	eae6320::cResult WriteTextureToFile(const char* const i_path_target, const DirectX::ScratchImage &i_texture)
	{
		...
		// Write the data for each MIP map
		{
			...
			const auto blockSize = GetSizeOfBlock(textureInfo.compressionType);
