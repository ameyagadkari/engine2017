...
function BuildAssets()
	...
	do
		...
		do
			local texture_authored = EngineSourceContentDir .. "Textures/dino_body.jpg"
			local texture_built = GameInstallDir .. "data/Textures/dino_body.btf"
			CreateDirectoryIfItDoesntExist( texture_built )
			local command = "\"" .. path_textureBuilder .. "\""
				.. " \"" .. texture_authored .. "\" \"" .. texture_built
			local result, exitCode = ExecuteCommand( command )
		...
		do
			local texture_authored = EngineSourceContentDir .. "Textures/default.tga"
			local texture_built = GameInstallDir .. "data/Textures/default.btf"
			CreateDirectoryIfItDoesntExist( texture_built )
			local command = "\"" .. path_textureBuilder .. "\""
				.. " \"" .. texture_authored .. "\" \"" .. texture_built
			local result, exitCode = ExecuteCommand( command )
	...
	do
		...
		do
			local mesh_authored = EngineSourceContentDir .. "Meshes/dino.tmf"
			local mesh_built = GameInstallDir .. "data/Meshes/dino.bmf"
			CreateDirectoryIfItDoesntExist( mesh_built )
			local command = "\"" .. path_meshBuilder .. "\""
				.. " \"" .. mesh_authored .. "\" \"" .. mesh_built
			local result, exitCode = ExecuteCommand( command )
	...
	-- Build the materials and copy them to the installation location
	do
		local path_materialBuilder = OutputDir .. "MaterialBuilder.exe"
		do
			local material_authored = EngineSourceContentDir .. "Materials/ball.tmaf"
			local material_built = GameInstallDir .. "data/Materials/ball.bmaf"
			CreateDirectoryIfItDoesntExist( material_built )
			local command = "\"" .. path_materialBuilder .. "\""
				.. " \"" .. material_authored .. "\" \"" .. material_built
			local result, exitCode = ExecuteCommand( command )
			if result then
				if exitCode == 0 then
					-- Display a message for each asset
					print( "Built " .. material_authored )
				else
					wereThereErrors = true
					-- The builder should already output a descriptive error message if there was an error
					-- (remember that you write the builder code,
					...
					-- but it can be helpful to still return an additional vague error message here
					-- in case there is a bug in the specific builder that doesn't output an error message
					OutputErrorMessage( "The command " .. command .. " failed with exit code " .. tostring( exitCode ), material_authored )
				end
			else
				wereThereErrors = true
				-- If the command wasn't executed then the second return value is an error message
				OutputErrorMessage( "The command " .. command .. " couldn't be executed: " .. tostring( exitCode ), material_authored )
			end
		end
		do
			local material_authored = EngineSourceContentDir .. "Materials/floor.tmaf"
			local material_built = GameInstallDir .. "data/Materials/floor.bmaf"
			CreateDirectoryIfItDoesntExist( material_built )
			local command = "\"" .. path_materialBuilder .. "\""
				.. " \"" .. material_authored .. "\" \"" .. material_built
			local result, exitCode = ExecuteCommand( command )
		...
		do
			local material_authored = EngineSourceContentDir .. "Materials/dino.tmaf"
			local material_built = GameInstallDir .. "data/Materials/dino.bmaf"
			CreateDirectoryIfItDoesntExist( material_built )
			local command = "\"" .. path_materialBuilder .. "\""
				.. " \"" .. material_authored .. "\" \"" .. material_built
			local result, exitCode = ExecuteCommand( command )
