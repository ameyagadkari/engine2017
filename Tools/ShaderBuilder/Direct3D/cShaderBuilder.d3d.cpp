...
namespace
{
	...
	public:
		virtual ~cIncludeHelper() = default;

		eae6320::cResult Initialize();
		explicit cIncludeHelper(const std::string& i_shaderSourcePath);
...
eae6320::cResult eae6320::Assets::cShaderBuilder::Build(const Graphics::ShaderTypes::eType i_shaderType, const std::vector<std::string>& i_arguments) const
{
	...
	// Load the source code
	{
		...
		if (!((result = LoadBinaryFile(m_path_source, dataFromFile, &errorMessage))))
		{
	...
	{
		std::string errorMessage;
		if (!((result = Platform::WriteBinaryFile(m_path_target, compiledCode->GetBufferPointer(), compiledCode->GetBufferSize(), &errorMessage))))
		{
...
namespace
{
	...
	eae6320::cResult cIncludeHelper::Initialize()
	{
		...
		{
			const auto posLastSlash = m_shaderSourcePath.find_last_of("/\\");
			if (posLastSlash != m_shaderSourcePath.npos)
			{
				m_shaderSourceDirectory = m_shaderSourcePath.substr(0, posLastSlash + 1);
			}
			else
			{
	...
	HRESULT cIncludeHelper::Open(const D3D_INCLUDE_TYPE i_includeType, const char* i_path, const void* i_parentData,
		const void** o_dataFromFile, unsigned int* o_dataSize)
	{
		...
		std::string pathToOpen;
		{
			...
			break;
			case D3D_INCLUDE_FORCE_DWORD: break;
			default: ;
			}
		}
		...
		{
			std::string errorMessage;
			const auto localResult = LoadBinaryFile(pathToOpen.c_str(), dataFromFile, &errorMessage);
			if (localResult)
			{
