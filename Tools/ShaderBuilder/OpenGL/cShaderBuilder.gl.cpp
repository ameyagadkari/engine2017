...
eae6320::cResult eae6320::Assets::cShaderBuilder::Build(const Graphics::ShaderTypes::eType i_shaderType, const std::vector<std::string>& i_arguments) const
{
	...
	if (!((result = SaveGeneratedShaderSource(m_path_target, shaderSourcePreProcessed))))
	{
		goto OnExit;
	}
	if (!((result = BuildAndVerifyGeneratedShaderSource(m_path_source, m_path_target, i_shaderType, shaderSourcePreProcessed))))
	{
		goto OnExit;
	}
...
namespace
{
		...
		const eae6320::Graphics::ShaderTypes::eType i_shaderType, const std::string& i_source)
	{
		...
		{
			std::string errorMessage;
			if (!((result = CreateHiddenContextWindow(hInstance, hiddenWindowInfo, &errorMessage))))
			{
				eae6320::Assets::OutputErrorMessageWithFileInfo(i_path_source, errorMessage.c_str());
				goto OnExit;
			}
		...
		// Compile the shader source code
		{
			...
			if (errorCode == GL_NO_ERROR)
			{
				...
				std::string compilationInfo;
				{
					...
					if (errorCode == GL_NO_ERROR)
					{
						const sLogInfo info(static_cast<size_t>(infoSize));
					...
					if (errorCode == GL_NO_ERROR)
					{
						if (didCompilationSucceed == GL_FALSE)
						{
							...
							if (s_glVendor != GlVendors::OTHER)
							{
								try
								{
									std::regex patternMatch;
									std::string patternReplace;
									{
										switch (s_glVendor)
										{
										case GlVendors::NVIDIA:
										{
											patternMatch.assign(R"(^\s*0\s*\(\s*(\d+)\s*\))");
											{
										...
										case GlVendors::AMD:
										{
											...
											*/
											...
											patternMatch.assign(R"(^\s*\w+\s*:\s*0\s*:\s*(\d+)\s*:\s*(\w+)\s*\(\s*(\S+)\s*\))");
											{
												std::ostringstream stringBuilder;
										...
										{
											patternMatch.assign(R"(^\s*(\w+)\s*:\s*0\s*:\s*(\d+)\s*:)");
											{
												std::ostringstream stringBuilder;
